...
from .forms import RegisterForm, AmbitoProyectoForm, ProjectPlanForm, TaskForm, RestriccionForm,  ResourceForm, ProjectRisksForm, WorkTeamMemberForm, EventoCronogramaForm, EsfuerzoProyectoForm
from .models import AmbitoProyecto, Task, ProjectPlan, Resource, ProjectRisks, WorkTeamMember, EventoCronograma, EsfuerzoProyecto
...
#project_plan = ProjectPlan.objects.get(id=project_id)
def ver_cronograma(request, project_id):
    # Filtra los eventos por el ID del proyecto y ordena por fecha de inicio proyecto_id=id
    eventos = EventoCronograma.objects.filter(project=project_id).order_by('fecha_inicio')
    # Renderiza la plantilla y pasa los eventos filtrados
...
def editar_evento(request, event_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('ver_cronograma', project_id=evento.project.id)
    else:
        form = EventoCronogramaForm(instance=evento)
    
    return render(request, 'cronograma/editar_evento.html', {'form': form, 'evento': evento})
#Pendiente agregar esfuerzo


def agregar_esfuerzo(request, project_id):
    proyecto = get_object_or_404(ProjectPlan, id=project_id)
    # Intenta obtener la instancia de EsfuerzoProyecto asociada, o crea una nueva
...
def consultar_costo(request, project_id):
    proyecto = get_object_or_404(ProjectPlan, id=project_id)
    contexto = {
